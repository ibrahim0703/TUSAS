...
class VIODataLoader:
    ...
    def get_frame_data(self):
        ...
        t_cam = row['timestamp']
        dt_seconds = (self.stereo_df.iloc[self.current_idx + 1]['timestamp'] - t_cam) / 1e9

        raw_left = cv.imread(os.path.join(CONFIG['CAM0_DIR'], str(row['filename_left'])), cv.IMREAD_GRAYSCALE)
        raw_right = cv.imread(os.path.join(CONFIG['CAM1_DIR'], str(row['filename_right'])), cv.IMREAD_GRAYSCALE)
        ...
        # IMU verilerini Kamera eksenine çevirerek al
        ...
        true_speed = np.sqrt(
            gt_row['v_RS_R_x [m s^-1]'] ** 2 + gt_row['v_RS_R_y [m s^-1]'] ** 2 + gt_row['v_RS_R_z [m s^-1]'] ** 2)

        self.current_idx += 1
        return rect_left, rect_right, dt_seconds, accel_cam, gyro_cam, true_speed
...
def main():
    ...
    print("[SİSTEM] VIO Basladi. Eksen Donusumleri (Coordinate Alignment) Devrede.")

    need_new_keyframe = True
    kf_obj_pts_3d = []
    kf_gyro_accum = np.zeros(3, dtype=np.float64)
    kf_time_elapsed = 0.0
    kf_R_WB = np.eye(3)  # DÜNYA EKSENİ REFERANSI (Başlangıçta Birim Matris)
    old_left = None
    old_p0 = None
    ...
    while True:
        ...
        # ---------------------------------------------------------------------
        # ESKF SÜREKLİ TAHMİN (PREDICT)
        # ---------------------------------------------------------------------
        filter_eskf.predict(accel_cam, gyro_cam, dt)

        # ---------------------------------------------------------------------
        # ANA KARE OLUŞTURMA VE DERİNLİK (KEYFRAME)
        # ---------------------------------------------------------------------
        if need_new_keyframe:
            ...
            if old_p0 is not None:
                ...
                # FİZİKSEL KİLİT 1: Ana karenin alındığı andaki Dünya Rotasyonunu mühürle
        ...
        # ---------------------------------------------------------------------
        # PNP ÇÖZÜMÜ VE ESKF GÜNCELLEME (UPDATE)
        # ---------------------------------------------------------------------
        if len(good_old_3d) >= 15:
            ...
            if success and kf_time_elapsed > 0:
                ...
                # 1. Kameranın kendi (yerel) eksenindeki hızı
                v_cam_measured = (t_cam_kf.flatten() / kf_time_elapsed)

                # FİZİKSEL KİLİT 2: EKSEN DÖNÜŞÜMÜ (Coordinate Transformation)
                # Kamera ekseninden -> Gövde (Body) Eksenine
                v_body_measured = R_CB.T @ v_cam_measured
                # Gövde ekseninden -> Dünya (World) Eksenine (Mühürlenmiş rotasyon ile)
                v_world_measured = kf_R_WB @ v_body_measured

                # Filtreye hizalanmış gerçek dünya hızını veriyoruz
                ...
                cv.putText(vis_frame, f"ESKF Hiz: {kalman_speed:.2f} m/s", (20, 30), cv.FONT_HERSHEY_SIMPLEX, 0.7,
                           (0, 255, 0), 2)
                cv.putText(vis_frame, f"GT Hiz  : {true_speed:.2f} m/s", (20, 60), cv.FONT_HERSHEY_SIMPLEX, 0.7,
                           (0, 0, 255), 2)
                cv.putText(vis_frame, f"HATA    : {error:.2f} m/s", (20, 90), cv.FONT_HERSHEY_SIMPLEX, 0.7,
                           (0, 255, 255), 2)
