...
============================================================
Drone anlık hiz tahmini: Stereo goru + IMU + 9D Kalman filtresi
...
def load_imu_data(imu_csv_path: str) -> dict:
    ...
    if not os.path.exists(imu_csv_path):
        print(f"[UYARI] IMU dosyası bulunamadi: {imu_csv_path}")
    ...
    print(f"[IMU] {len(imu_data)} örnek yuklendi.")
    return imu_data
...
# ─────────────────────────────────────────────────────────────────────────────
def main():
    print("[SYSTEM] VIO Pipeline baslatiliyor (Stereo + IMU + 9D Kalman)...")
    ...
    if len(left_images) != len(right_images):
        print(f"[UYARI] Sol/sağ goruntu sayısı eslesmedi: "
    ...
    print("[SYSTEM] Hiz tahmini: Kalman filtreli (9D state)")
    ...
    for i in range(n_frames - 1):
        ...
        if dt <= 0:
            print(f"[UYARI] Frame {i}: dt <= 0, atlaniyor.")
            continue
        ...
        # ─────────────────────────────────────────────────────────────────────
        # ADIM 3: t1 sol goruntuyü rectify et
        # ─────────────────────────────────────────────────────────────────────
        ...
        # ─────────────────────────────────────────────────────────────────────
        # ADIM 8: Hiz kontrolü (fiziksel sınır filtresi)
        # ─────────────────────────────────────────────────────────────────────
        ...
        # Ilerleme çıktısı (her 500 frame'de bir)
        if n_success % 500 == 0:
    ...
    print(f"  Başarı orani       : {100 * n_success / max(n_total, 1):.1f}%")
    print("=" * 55)
    print("  Cikti: estimated_trajectory.csv")
    print("  Sutunlar: timestamp, vx, vy, vz, speed_m_s, inliers, imu_fused")
    print("=" * 55)
