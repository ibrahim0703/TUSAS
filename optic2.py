...
GT_FREQ = 20.0                         # Hız verisinin frekansı

# Başlangıç Fiziksel Tahminleri
FOCAL_LENGTH = 1000.0 
HEIGHT_Z = 1.5       
...
def get_roi_mask(frame):
    ...
    # --- MASKE AYARLARI ---
    # Yamuk (Trapezoid) şeklinde bir alan.
    # pts dizisinin şekli (N, 1, 2) olmak zorundadır.
    pts = np.array([
    ...
    pts = pts.reshape((-1, 1, 2)) # OpenCV'nin istediği format
...
try:
    ...
    # Kod patlamasın diye sahte veri oluşturuyoruz (Test için)
...
roi_mask, roi_pts = get_roi_mask(old_frame) # MASKEYİ OLUŞTUR
...
try:
    while True:
        ...
        if p1 is not None:
            ...
            dy = displacement[:, 1] # Dikey hareket
            ...
            for i, (new, old) in enumerate(zip(good_new, good_old)):
                ...
                vis_frame = cv.line(vis_frame, (int(a), int(b)), (int(c), int(d)), (0, 255, 0), 2)
                vis_frame = cv.circle(vis_frame, (int(a), int(b)), 3, (0, 0, 255), -1)
...
plt.plot(est_t_crop, est_v_crop, 'b--', label='Senin Kodun (Ham)')
plt.title('Ham Veri')
plt.legend()
plt.grid()
...
plt.plot(gt_t_crop, est_v_calibrated, 'g-', label='Senin Kodun (Kalibre)')
plt.title(f'KALİBRE EDİLMİŞ (RMSE: {rmse:.2f})')
plt.legend()
plt.grid()
